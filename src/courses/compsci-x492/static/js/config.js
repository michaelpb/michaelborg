...
modulo.register('util', function parseMarkdown (source) {
    ...
    html = html.replace(/\\resumeNoSpace/gi, 'résumé'); // Swap out 
    html = html.replace(/\\resume/gi, 'résumé'); // Swap out 
    html = html.replace(/\\Resume/gi, 'Résumé'); // Swap out 
    html = html.replace(/<p>\\blankline<.p>/gi, '<div style="border-bottom: 2px dashed gray">&gt;&nbsp;&nbsp;</div>\n'); // Swap out 
    html = html.replace(/<p>\\newpage<.p>/gi, '<div style="height: 100px"></div>\n'); // Swap out 
